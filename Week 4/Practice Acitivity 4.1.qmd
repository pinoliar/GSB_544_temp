...
for row in table.find_all("tr")[2:]:
    ...
    population_2024 = cells[2].get_text(strip = True)
    land_area_sq_mi = cells[5].get_text(strip = True)
    rows.append({
      "City": city,
      "State": state,
      "Population (2024 estimate)": population_2024,
...
for block in course_blocks:
    cells = block.find("p", class_="courseblocktitle")
    course = cells.get_text(strip=True)
    cells = block.find("p", class_="noindent")
    term = cells.get_text(strip=True).replace("Term Typically Offered:", "")
    rows.append({
